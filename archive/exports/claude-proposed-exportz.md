...
**🚀 Production Status:** Generic Agent Orchestrator (Template)
...
* **Backlog items**: Use `.claude/skills/backlog-management/add-backlog-item.sh` when deferring work (requires: item, why, solution)
* **Changelog entries**: Use `.claude/skills/changelog-management/add-changelog-entry.sh` after feature/bugfix/breaking change (10-100 char summary, sparse impact)
...
* **For multi-agent workflows**: Use coordination commands (automatically handles coordinator spawning)
...
* **Ruthlessly delegate** - if task requires >3 steps, use coordination commands immediately
...
**Core Skills:**
- Coordination Protocols (`.claude/skills/coordination/SKILL.md`)
- Agent Spawning (`.claude/skills/agent-spawning/SKILL.md`)
- Loop Validation (`.claude/skills/loop-validation/SKILL.md`)
...
**Structure:**
- Agents: `.claude/agents/[team-name]/` (production agents)
- Skills: `.claude/skills/` (skills, configurable prefix)
- Hooks: `.claude/hooks/` (hooks, configurable prefix)
- Commands: `.claude/commands/` (commands, subdirectory)

**Installation:**
```bash
npm install [package-name]
npx [init-command]  # Copy namespace-isolated files
```

**Collision Risk:** ~0.01% (user custom files preserved)

**Package:** Configurable size (adjust based on implementation)

**Agent Discovery:** Recursive search through `.claude/agents/**/*.md` finds both team agents and user custom agents
...
* Coordinator handles all agent spawning internally using orchestration skills
* Delegate ALL coordination to skills
* Use skill-specific configuration for complex workflows

### Execution Modes (Template)

**Available modes:**

**1. CLI Mode (Production):**
```bash
/[cli-command] "Task description" --mode=standard
```
...
```bash
/[task-command] "Task description" --mode=standard
```
- Main Chat spawns all agents via Task()
- NO coordinator agent
- Higher cost for full visibility
- Use: Debugging, learning, short tasks (<5 min)

**Mode selection guidance:**
- "execute loop on X" → `[task-command]` (default)
- "use task mode on X" → `[task-command]`
- "use cli mode on X" → `[cli-command]`
- "production loop on X" → `[cli-command]`

**Architecture patterns:**
- CLI: Main Chat → coordinator → orchestrate.sh → CLI workers (background)
- Task: Main Chat → Task() agents (no coordinator, full visibility)

**Cost breakdown:**
- CLI mode: Optimized for production (configurable provider routing)
- Task mode: Higher cost for full visibility
...
```bash
BACKUP_PATH=$(./[hooks-directory]/invoke-pre-edit.sh "$FILE_TO_EDIT" --agent-id "$AGENT_ID")
```
...
**Injection:** Automatically included in all agent prompts via `cli/agent-prompt-builder.ts`
...
./[skills-directory]/pre-edit-backup/revert-file.sh "$FILE_PATH" --agent-id "$AGENT_ID"
```
...
```bash
./[hooks-directory]/invoke-post-edit.sh "$EDITED_FILE" --agent-id "$AGENT_ID"
```

**Why:** Prevents errors and disorganization from propagating. Non-blocking by default.
**Config:** `[hooks-directory]/post-edit.config.json`
**Skill:** `[skills-directory]/hook-pipeline/SKILL.md`

### Complete Edit Workflow (Pre-Edit + Edit + Post-Edit)
```bash
# 1. Pre-Edit: Create backup
BACKUP_PATH=$(./[hooks-directory]/invoke-pre-edit.sh "src/file.ts" --agent-id "$AGENT_ID")

# 2. Edit: Perform file modification
Edit: file_path="src/file.ts" old_string="..." new_string="..."

# 3. Post-Edit: Validate changes
./[hooks-directory]/invoke-post-edit.sh "src/file.ts" --agent-id "$AGENT_ID"

# 4. (Optional) Revert if validation fails
if [ $? -ne 0 ]; then
    ./[skills-directory]/pre-edit-backup/revert-file.sh "src/file.ts" --agent-id "$AGENT_ID"
fi
```

## 2) When Agents Are Mandatory (Triggers)

If **any** apply, use coordination CLI commands:
...
/[cli-command] "Complex task description" --mode=standard

# Debugging with full visibility
/[task-command] "Complex task description" --mode=standard
```

**Do NOT manually spawn Task() agents for complex workflows - the CLI commands handle coordination automatically.**
...
npx [package-name] swarm "Task Description" \
...
**🚨 FOR COMPLEX WORKFLOWS: Always use CLI commands - never manual Task() spawning**
```bash
# Multi-agent workflows (coordinator handles everything)
/[cli-command] "Build authentication system" --mode=standard
...
Refer to `[skills-directory]/coordination/SKILL.md` for:
...
- Coordination via `[skills-directory]/loop-validation/SKILL.md`
...
**CLI Mode (Production) - Enhanced:**
Main Chat spawns coordinator → Enhanced orchestrator with monitoring → Workers via CLI with progress tracking → Automatic recovery from stuck agents
...
```bash
./[skills-directory]/loop-orchestration/orchestrate.sh
```
...
- Spawns implementation agents with protocol enforcement
- Enhanced waiting with progress tracking and recovery
- Collects confidence scores with metadata validation
- Gate check: spawn validators if ≥threshold (with health verification)
- Spawns validator agents with monitoring
...
./[skills-directory]/coordination/report-completion.sh \
  --task-id "$TASK_ID" \
  --agent-id "$AGENT_ID" \
  --confidence 0.85 \
  --iteration 1 \
...
**Orchestration Flow:**
1. Implementation agents complete work and report confidence
2. **Gate Check:** Implementation self-validation scores checked
   - IF gate FAILS → Wake implementation agents for iteration N+1 (skip validators)
   - IF gate PASSES → Signal validators to start work
3. Validators WAIT for gate pass signal (coordination-wait "swarm:${TASK_ID}:gate-passed")
4. Validators review implementation work and report consensus
5. **Product Owner Decision:** Orchestrator spawns Product Owner and parses output
   - Uses `[skills-directory]/product-owner-decision/execute-decision.sh`
...
**Recommended: Use coordination commands for loop execution**

Main Chat should use these commands instead of manually spawning coordinators.
**IMPORTANT** Subagents must NOT use these slash commands

**Single Task:**
```bash
/[cli-command] "Implement JWT authentication" --mode=standard
/[task-command] "Fix security bug in auth module" --mode=standard
```

**What These Commands Do:**
- Automatically spawn coordinator
- Pass structured parameters (success criteria, agent configuration)
- Include cost optimization reminders
...
**See:** `[commands-directory]/COORDINATOR_PARAMETERS.md` for detailed parameter specifications
...
Implement comprehensive test suites that validate both functional requirements and edge cases, including timeout scenarios and blocking mechanism effectiveness. Example: `[skills-directory]/coordination/test-orchestrator.sh` validates coordination blocking, agent completion protocol, and consensus collection with targeted tests.

## 6) Additional Resources

**Skill References:**
- Coordination Protocols: `[skills-directory]/coordination/SKILL.md`
- Agent Spawning: `[skills-directory]/agent-spawning/SKILL.md`
- Loop Validation: `[skills-directory]/loop-validation/SKILL.md`

**Loop Documentation:**
- **Task Mode Guide**: `[commands-directory]/LOOP_TASK_MODE.md` (agent specialization, workflow, backlog management, adaptive validator scaling)
- Coordinator Parameters: `[commands-directory]/COORDINATOR_PARAMETERS.md`
...
- **Tags**: context-injection, coordination, multi-layer, deliverables
- **Confidence**: 0.92
- **Priority**: 9/10
...
- **Insight**: Avoid storing context in persistence layer without retrieving and injecting it into agent prompts. Anti-pattern: Orchestrator stores epic-context, phase-context, success-criteria in persistence but spawns agents with generic context ('implementation for iteration N'). Result: Agents have no access to deliverables list, acceptance criteria, or directory paths despite context being 'available'.
...
- **Insight**: Never pass generic iteration-level context when task-specific deliverables exist. Anti-pattern: Agent receives 'implementation for iteration 2' when persistence contains detailed deliverables list. Agents have no telepathy - they cannot infer '[skills-directory]/checkpoint-state/SKILL.md' from 'iteration 2'. Always inject complete deliverables, directory paths, and acceptance criteria even if 'already available'.
...
- **Context**: Loop Validation (Task Mode)
- **Insight**: Main Chat spawns validators via Task() without clear scope boundaries or mode-aware completion protocols. Anti-pattern: Validators see CLI-mode completion instructions (coordination signals, `invoke-waiting-mode.sh report`) and attempt to comply using wrong tools (slash commands via Bash, nested loop spawning). Result: Agents hang indefinitely trying to execute `[cli-command]` as bash command, memory leak from blocked processes. Task-spawned validators should simply return structured output - Main Chat receives it automatically, no coordination signals needed.
- **Tags**: task-spawning, validation, completion-protocol, scope-boundaries, memory-leak
- **Confidence**: 0.95
- **Priority**: 10/10
- **Fix**: Mode-specific completion protocols and validator agents. Explicit scope boundaries prevent nested loop spawning.