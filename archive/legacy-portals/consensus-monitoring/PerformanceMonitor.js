...
export class PerformanceMonitor extends EventEmitter {
  ...
  getAlertSeverity(type, data) {
    const severityMap = {
      ...
      'response-time':
        data.duration > this.options.alertThresholds.responseTime * 2 ? 'critical' : 'warning',
      'slow-consensus':
        data.duration > this.options.alertThresholds.consensusTime * 1.5 ? 'critical' : 'warning',
    };
  ...
  getMetrics(type, timeRange = 60000) {
  ...
  generateReport(timeRange = 300000) {
  ...
  generateRecommendations(metrics, status) {
    ...
    if (status.recentMetrics.memory > 400 * 1024 * 1024) {
      // 400MB
      recommendations.push({
        type: 'memory',
        priority: 'high',
        message:
          'High memory usage detected. Check for memory leaks and consider garbage collection tuning.',
    ...
    if (status.recentMetrics.responseTime > 3000) {
      recommendations.push({
        ...
        priority: 'medium',
        message:
          'High response times detected. Consider caching, database optimization, or async processing.',
  ...
  simulateLoad(options = {}) {
    const { duration = 10000, cpuLoad = 50, memoryMB = 100, requestsPerSecond = 10 } = options;
