...
export class ConsensusTracker extends EventEmitter {
  ...
  private readonly criticalSuccessRate = 0.9;
  ...
  private trackParticipantResponse(data: {
    ...
    const now = new Date();
    if (
      !decision.timing.firstResponseAt ||
      decision.timing.firstResponseAt.getTime() === decision.timing.initiatedAt.getTime()
    ) {
  ...
  private trackDecisionCompletion(data: {
    ...
    decision.timing.avgResponseTime =
  ...
  private updateParticipantMetrics(
    ...
    const prevTotal =
      participant.performance.averageResponseTime * (participant.performance.totalDecisions - 1);
    participant.performance.averageResponseTime =
      (prevTotal + response.responseTime) / participant.performance.totalDecisions;

    // Update confidence level
    const prevConfidenceTotal =
      participant.performance.confidenceLevel * (participant.performance.totalDecisions - 1);
    participant.performance.confidenceLevel =
      (prevConfidenceTotal + response.confidence) / participant.performance.totalDecisions;

    // Update voting patterns
    const existingPattern = participant.behaviorAnalysis.votingPatterns.find(
      (p) => p.vote === response.vote,
    );
    if (existingPattern) {
  ...
  private updateDecisionPerformanceMetrics(decision: ConsensusDecision): void {
    ...
    if (recentDecisions.length > 0) {
      ...
      const consensusAchieved = recentDecisions.filter(
        (d) => d.outcome.decision !== 'failed' && d.outcome.decision !== 'escalated',
      ).length;
    ...
    const decisionsLastMinute = this.throughputHistory.filter(
      (t) => t.timestamp >= lastMinute,
    ).length;
  ...
  private updatePerformanceMetrics(): void {
    ...
    const byzantineDecisions = recentDecisions.filter(
      (d) => d.byzantineMetrics.suspectedFaults > 0,
    );
    const recoveredDecisions = byzantineDecisions.filter((d) => d.outcome.decision !== 'failed');
    metrics.reliability.faultTolerance =
      byzantineDecisions.length > 0 ? recoveredDecisions.length / byzantineDecisions.length : 1.0;
  ...
  private detectByzantinePatterns(): void {
    ...
    const patterns = this.byzantineDetector.detectPatterns(
      recentDecisions,
    ...
    Array.from(this.participants.values()).forEach((participant) => {
      ...
      if (analysis.suspectedByzantine && !participant.behaviorAnalysis.suspectedByzantine) {
        this.createAlert(
          ...
          [
            'Investigate participant behavior',
            'Review recent decisions',
            'Consider participant exclusion',
          ],
        );
      }
    });
  ...
  private checkPerformanceAlerts(decision: ConsensusDecision): void {
    ...
    if (decision.timing.totalDuration > this.maxDecisionTime) {
      this.createAlert(
        ...
        [],
        [
          'Investigate slow participants',
          'Review decision complexity',
    ...
    if (decision.byzantineMetrics.suspectedFaults > 0) {
      this.createAlert(
        'byzantine_detected',
        decision.byzantineMetrics.suspectedFaults > 2 ? 'critical' : 'warning',
  ...
  private checkPerformanceThresholds(metrics: ConsensusPerformanceMetrics): void {
    ...
    if (metrics.quality.byzantineFailureRate > 0.05) {
      // 5% threshold
      this.createAlert(
        'byzantine_detected',
        'warning',
    ...
    if (metrics.timing.averageDecisionTime > this.targetResponseTime * 2) {
      this.createAlert(
        ...
        [
          ...
          'Review participant responsiveness',
        ],
      );
    }
  }
  ...
  private calculateAverageThroughput(decisions: ConsensusDecision[]): number {
    ...
    const timespan =
      sortedDecisions[sortedDecisions.length - 1].timing.decidedAt.getTime() -
  ...
  private calculateParticipantReliability(): number {
    ...
    const totalReliability = participants.reduce(
      (sum, p) => sum + p.performance.reliabilityScore,
      0,
    );
  ...
  getParticipantMetrics(): ParticipantMetrics[] {
    return Array.from(this.participants.values()).sort(
      (a, b) => b.performance.totalDecisions - a.performance.totalDecisions,
    );
  }
  ...
  getConsensusHealthScore(): {
    ...
    const components = {
      ...
      responseTime: Math.max(
        0,
        1 - metrics.timing.averageDecisionTime / (this.targetResponseTime * 2),
      ),
    ...
    const overall =
      (components.successRate +
        ...
        components.byzantineTolerance +
        components.participantReliability) /
      4;
...
class ByzantineDetector {
  ...
  analyzeFinalDecision(decision: ConsensusDecision): any {
    ...
    analysis.reliabilityScore = Math.max(
      0,
      1 - analysis.suspectedFaults * 0.2 - analysis.detectedInconsistencies * 0.1,
    );
  ...
  detectPatterns(decisions: ConsensusDecision[], participants: ParticipantMetrics[]): any[] {
    ...
    participants.forEach((participant) => {
      ...
      participantDecisions.forEach((decision) => {
        const participantVote = decision.participants.find(
        ...
        if (voteCount / allVotes.length < 0.4) {
          // Minority vote
          minorityVotes++;
        }
      });

      if (minorityVotes / participantDecisions.length > 0.7) {
        // 70% minority votes
        patterns.push({
  ...
  private detectCoordinatedBehavior(
    ...
    for (let i = 0; i < participants.length; i++) {
      for (let j = i + 1; j < participants.length; j++) {
        ...
        const sharedDecisions = decisions.filter(
          (d) =>
        ...
        sharedDecisions.forEach((decision) => {
          const vote1 = decision.participants.find(
            (p) => p.id === participant1.participantId,
          )?.vote;
          const vote2 = decision.participants.find(
            (p) => p.id === participant2.participantId,
          )?.vote;
  ...
  analyzeParticipantBehavior(participant: ParticipantMetrics): {
    ...
    if (participant.performance.averageResponseTime > 10000) {
      // > 10 seconds
    ...
    const totalVotes = participant.behaviorAnalysis.votingPatterns.reduce(
      (sum, p) => sum + p.frequency,
      0,
    );
    const entropy = this.calculateEntropy(
      participant.behaviorAnalysis.votingPatterns.map((p) => p.frequency / totalVotes),
    );

    if (entropy < 0.5) {
      // Low entropy = always same vote
    ...
    const daysSinceActive =
      (Date.now() - participant.behaviorAnalysis.lastActiveTime.getTime()) / (1000 * 60 * 60 * 24);
