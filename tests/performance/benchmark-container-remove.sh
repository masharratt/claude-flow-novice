...
RESULTS_FILE=".artifacts/benchmarks/benchmark-remove-latency.txt"
...
echo "Container Remove Latency Benchmark"
echo "==================================="
...

{
    ...
    [ -n "$TEST_CONTAINER_ID" ] && docker rm -f "$TEST_CONTAINER_ID" 2>/dev/null || true
    sleep 1
    ...
    # Calculate expected proxy overhead (5-8ms is typical for removal)
    proxy_overhead=6
