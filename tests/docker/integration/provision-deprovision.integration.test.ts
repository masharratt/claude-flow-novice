...
if (DOCKER_AVAILABLE) {
  ...
  const runTests = async () => {
    const { ProvisionTeam } = await import('../../../src/docker/scripts/provision-team');
    const { DeprovisionTeam } = await import('../../../src/docker/scripts/deprovision-team');

    describe('Provision/Deprovision Integration', () => {
      ...
      afterAll(async () => {
        ...
        try {
          execSync(
            `docker ps -a --filter "name=${testTeamId}" -q | xargs -r docker rm -f`,
            { stdio: 'ignore' }
          );
          execSync(
            `docker network ls --filter "name=${testTeamId}" -q | xargs -r docker network rm`,
            { stdio: 'ignore' }
          );
        } catch {
          // Ignore
      ...
      describe('DeprovisionTeam', () => {
        beforeEach(async () => {
          ...
          try {
            execSync(`docker network create ${testTeamId}-network || true`, {
              stdio: 'ignore',
            });
          } catch {
            // Ignore
      ...
      describe('Provision/Deprovision lifecycle', () => {
        it('should complete full provision-deprovision cycle', async () => {
          ...
          const remainingContainers = execSync(
            `docker ps -a --filter "name=${testTeamId}" -q`
          ).toString();
