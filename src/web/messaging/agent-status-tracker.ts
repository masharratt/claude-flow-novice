...
export class AgentStatusTracker extends EventEmitter {
  ...
  public updateAgentStatus(update: Partial<AgentStatus> & { id: string; swarmId: string }): void {
    ...
    const existing = this.agentStatuses.get(update.id) || this.createDefaultAgentStatus(update.id, update.type!);
   ...
   */
  public updateTaskProgress(agentId: string, swarmId: string, progress: {
    taskId: string;
    description: string;
    progress: number;
    estimatedCompletion?: string;
   ...
   */
  public updateCoordination(swarmId: string, coordination: {
    fromAgent: string;
    toAgent: string;
    ...
    switch (coordination.action) {
      ...
      case 'unblock':
        fromAgent.coordination.blocking = fromAgent.coordination.blocking.filter(id => id !== coordination.toAgent);
        toAgent.coordination.blockedBy = toAgent.coordination.blockedBy.filter(id => id !== coordination.fromAgent);
  ...
  public getSwarmDashboard(swarmId: string): any {
    ...
    return {
      ...
      relaunchInfo: swarmStatus.relaunchHistory ? {
        ...
        lastAttempt: swarmStatus.relaunchHistory.attempts[swarmStatus.relaunchHistory.attempts.length - 1]
      } : null
    };
  }
  ...
  private createDefaultAgentStatus(id: string, type: 'researcher' | 'coder' | 'reviewer'): AgentStatus {
    return {
      id,
      type,
      status: 'idle',
  ...
  private calculateCoordinationMetrics(agents: AgentStatus[]): SwarmStatus['coordination'] {
    const totalHandoffs = agents.reduce((sum, agent) => sum + agent.coordination.pendingHandoffs, 0);
    const totalEvents = agents.reduce((sum, agent) => sum + agent.performance.coordinationEvents, 0);
    ...
    for (const agent of agents) {
      ...
      if (agent.coordination.pendingHandoffs > 2) {
        bottlenecks.push(`${agent.id} has high handoff queue (${agent.coordination.pendingHandoffs})`);
      }
    }

    return {
      activeHandoffs: totalHandoffs,
      coordinationEfficiency: totalEvents > 0 ? Math.min(100, (totalEvents / agents.length) * 10) : 100,
  ...
  private calculatePerformanceMetrics(agents: AgentStatus[], existingSwarm?: SwarmStatus): SwarmStatus['performance'] {
    const completedTasks = agents.reduce((sum, agent) => sum + agent.performance.tasksCompleted, 0);
    const averageTaskTime = agents.length > 0
      ? agents.reduce((sum, agent) => sum + agent.performance.averageTaskTime, 0) / agents.length
      : 0;

    const efficiency = agents.length > 0
      ? agents.reduce((sum, agent) => sum + agent.performance.successRate, 0) / agents.length * 100
      : 0;
  ...
  private generateCommunicationMatrix(agents: AgentStatus[]): Record<string, Record<string, number>> {
    ...
    for (const agent of agents) {
      ...
      for (const otherAgent of agents) {
        if (agent.id !== otherAgent.id) {
          matrix[agent.id][otherAgent.id] = agent.coordination.connectedAgents.includes(otherAgent.id) ? 1 : 0;
        }
      }
    }
  ...
  private generateStatusAlerts(swarmStatus: SwarmStatus): string[] {
    ...
    if (swarmStatus.coordination.activeHandoffs > 5) {
      alerts.push(`High coordination load (${swarmStatus.coordination.activeHandoffs} pending handoffs)`);
    }
  ...
  private calculateAverageTaskTime(agents: AgentStatus[]): number {
    ...
    return taskTimes.length > 0 ? taskTimes.reduce((sum, time) => sum + time, 0) / taskTimes.length : 0;
  }
  ...
  private calculateProductivityTrend(agents: AgentStatus[]): string {
    ...
    const averageSuccess = agents.reduce((sum, agent) => sum + agent.performance.successRate, 0) / agents.length;
  ...
  public getActiveSwarmCount(): number {
    return Array.from(this.swarmStatuses.values()).filter(swarm => swarm.status === 'active').length;
  }
}