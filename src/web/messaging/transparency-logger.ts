...
export class TransparencyLogger extends EventEmitter {
  ...
  private generateTransparencyAlerts(logs: TransparencyLog[]): string[] {
    ...
    const decisionsWithoutReasoning = logs.filter(log =>
      log.type === 'decision' && !log.metadata.reasoning
    );
    ...
    const lowConfidenceDecisions = logs.filter(log =>
      log.type === 'decision' && log.metadata.confidence && log.metadata.confidence < 0.5
    );
  ...
  private async ensureLogDirectory(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to create transparency log directory', { error, path: this.logPath });
    }
  }
