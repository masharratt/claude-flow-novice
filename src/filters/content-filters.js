...
class ContentFilters {
  ...
  async analyzeRequest(request) {
    ...
    if (path.endsWith('.md')) {
      const isAutoGenPattern = this.config.autoGenPatterns.some(
        (pattern) => path.toLowerCase().includes(pattern.toLowerCase()) || path.endsWith(pattern),
      );
  ...
  isAutoGeneratedPath(path) {
    ...
    return autoGenPathPatterns.some((pattern) => lowerPath.includes(pattern));
  }
