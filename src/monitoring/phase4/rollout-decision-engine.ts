...
export class RolloutDecisionEngine extends EventEmitter {
  ...
  constructor(logger: ILogger, eventBus: IEventBus, customCriteria?: Partial<DecisionCriteria>) {
    super();
    this.logger = logger;
  ...
  private calculateMetricScores(): {
    ...
    if (this.currentMetrics.phase4) {
      ...
      scores.validationAccuracy =
        validScores.length / metrics.truthScoreDistribution.scores.length || 0;
    ...
    if (this.currentMetrics.performance) {
      ...
      scores.systemPerformance = Math.max(
        0,
        1 -
  ...
  private generateRecommendation(
    ...
    if (
      thresholds.failed.includes('validation_accuracy_minimum') ||
  ...
  private assessDecisionImpact(
    ...
    switch (recommendation) {
      case 'proceed':
        ...
        break;
      case 'pause':
        affectedUsers = Math.floor(baseUsers * 0.1); // Current 10%
        break;
      case 'rollback':
        affectedUsers = Math.floor(baseUsers * 0.1); // Rolling back 10%
        break;
    }
    ...
    const rollbackComplexity: 'simple' | 'moderate' | 'complex' =
      this.rolloutProgress.currentCohort.percentage < 15
        ? 'simple'
        : this.rolloutProgress.currentCohort.percentage < 50
          ? 'moderate'
          : 'complex';
  ...
  private updateRolloutProgress(decision: RolloutDecision): void {
    ...
    const progressRate =
      decision.recommendation === 'proceed' ? 1.2 : decision.recommendation === 'pause' ? 0.5 : 0;

    this.rolloutProgress.timeline.remaining =
      Math.max(0, this.rolloutProgress.timeline.estimated - elapsed) * (2 - progressRate);
