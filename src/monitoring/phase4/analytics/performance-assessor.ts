...
export class SystemPerformanceAssessor extends EventEmitter {
  ...
  private analyzeValidationPerformance(): void {
    ...
    if (
      validation.latency.trend === 'degrading' &&
      validation.latency.average > this.thresholds.latency.warning
    ) {
      this.createAlert(
        'performance_degradation',
        'warning',
    ...
    if (hooks.executionMetrics.p95ExecutionTime > this.thresholds.hookExecution.critical) {
      this.createAlert(
        ...
        `Hook P95 execution time high: ${hooks.executionMetrics.p95ExecutionTime.toFixed(1)}ms`,
        {
          ...
          threshold: this.thresholds.hookExecution.critical,
          deviation:
            hooks.executionMetrics.p95ExecutionTime - this.thresholds.hookExecution.critical,
        },
      );
    }
  }
  ...
  private compareToBaseline(): void {
    ...
    if (Math.abs(deviation.responseTime) > 1000) {
      // 1 second deviation
      this.createAlert(
        'performance_degradation',
        'warning',
  ...
  private getRecentMetricHistory(
    metric: string,
    windowMs: number,
  ...
  getSystemHealthScore(): {
    ...
    const components = {
      validation: Math.max(
        0,
        1 - metrics.validation.latency.current / (this.thresholds.latency.critical * 2),
      ),
      resources: Math.max(0, 1 - metrics.system.resourceUtilization.cpu.usage / 100),
      errors: Math.max(
        0,
        1 - metrics.system.errorMetrics.errorRate / this.thresholds.errorRate.critical,
      ),
      hooks: Math.max(
        0,
        1 -
          metrics.hooks.executionMetrics.averageExecutionTime /
            (this.thresholds.hookExecution.critical * 2),
      ),
    };

    const overall =
      (components.validation + components.resources + components.errors + components.hooks) / 4;
