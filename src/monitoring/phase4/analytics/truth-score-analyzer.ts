...
export class TruthScoreAnalyzer extends EventEmitter {
  ...
  // Thresholds
  private readonly accuracyThreshold = 0.90; // 90% target
  private readonly warningThreshold = 0.85;
  private readonly criticalThreshold = 0.80;
  ...
  private updateRunningStatistics(score: TruthScore): void {
    ...
    if (baselineModel) {
      ...
      baselineModel.mean = previousMean + ((isValid ? 1 : 0) - previousMean) / baselineModel.sampleSize;
  ...
  private updateDomainStatistics(score: TruthScore): void {
    ...
    domainModel.mean = domainModel.scores.reduce((sum, s) => sum + s, 0) / domainModel.scores.length;
  }
  ...
  private updateComplexityStatistics(score: TruthScore): void {
    ...
    complexityModel.mean = complexityModel.scores.reduce((sum, s) => sum + s, 0) / complexityModel.scores.length;
  }
  ...
  private checkImmediateAlerts(score: TruthScore): void {
    ...
    if (score.metadata.disagreementCount > 2) {
      this.createAlert('consensus_failure', 'warning',
  ...
  private checkValidationBias(): void {
    ...
    if (maxAccuracy - minAccuracy > 0.2) { // 20% difference
  ...
  private performTrendAnalysis(): void {
    try {
      ...
      if (trends.overall === 'declining' && prediction.accuracy < this.accuracyThreshold) {
        this.createAlert('pattern_anomaly', 'critical',
  ...
  private analyzeCohortPatterns(scores: TruthScore[]): void {
    ...
    if (cohortA.length > 20 && cohortB.length > 20) {
      ...
      if (Math.abs(accuracyA - accuracyB) > 0.1) {
        this.createAlert('pattern_anomaly', 'warning',
  ...
  private handleAnomalies(anomalies: any[]): void {
    anomalies.forEach(anomaly => {
      this.createAlert('pattern_anomaly', anomaly.severity || 'warning',
  ...
  private detectPatternChanges(scores: TruthScore[]): void {
    ...
    currentPatterns.forEach(currentPattern => {
      ...
      if (existingPattern) {
        ...
        if (accuracyChange > 0.05) { // 5% change threshold
          this.createAlert('pattern_anomaly', 'warning',
  ...
  getValidationAccuracyMetrics(): ValidationAccuracyMetrics {
    ...
    // By domain
    const byDomain = Object.entries(this.groupBy(recentScores, s => s.context.domain))
      .map(([domain, scores]) => ({
        domain,
        accuracy: this.calculateAccuracy(scores as TruthScore[]),
    ...
    // By complexity
    const byComplexity = Object.entries(this.groupBy(recentScores, s => s.context.complexity))
    ...
    // By validation type
    const byValidationType = Object.entries(this.groupBy(recentScores, s => s.validationType))
      .map(([type, scores]) => ({
        type,
        accuracy: this.calculateAccuracy(scores as TruthScore[]),
        sampleSize: scores.length
