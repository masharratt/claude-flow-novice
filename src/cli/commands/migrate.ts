...
async function showMigrationStatus(projectPath: string): Promise<void> {
  ...
  if (!analysis.hasOptimizedPrompts) {
    console.log(chalk.bold('\n💡 Recommendations:'));
    console.log('  • Run migration analysis: claude-flow-novice migrate analyze');
    console.log('  • Start with dry run: claude-flow-novice migrate --dry-run');
    console.log('  • Use selective strategy: claude-flow-novice migrate --strategy selective');
  }
