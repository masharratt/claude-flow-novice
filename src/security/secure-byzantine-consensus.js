...
class SecureByzantineConsensus extends EventEmitter {
  ...
  generateSecureNodeId() {
    ...
    const hash = crypto.createHash('sha256').update(entropy).update(timestamp).digest('hex');
  ...
  evaluateSecureConsensus(validVotes, proposalHash) {
    ...
    consensusProof.proofSignature = crypto
      .sign(
        'RSA-PSS',
        Buffer.from(
          JSON.stringify({
  ...
  createAuditRecord(proposal, votes, consensusResult) {
    ...
    // Sign audit record
    auditRecord.auditSignature = crypto
      .sign('RSA-PSS', Buffer.from(auditRecord.recordHash), {
        key: this.keyPair.privateKey,
        padding: crypto.constants.RSA_PKCS1_PSS_PADDING,
        saltLength: crypto.constants.RSA_PSS_SALTLEN_DIGEST,
        hashAlgorithm: 'sha384',
  ...
  logSecurityEvent(eventType, eventData) {
    ...
    logEntry.signature = crypto
      .createHmac('sha384', this.keyPair.privateKey)
