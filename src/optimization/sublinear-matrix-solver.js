...
class SublinearMatrixSolver extends EventEmitter {
    ...
    async generatePerformanceCertificate(data) {
        ...
        const digitalSignature = crypto.createHmac('sha256', this.nodeId + 'cert-secret')
    ...
    async generateCryptographicProof(data) {
        ...
        const signature = crypto.createHmac('sha256', this.nodeId + 'proof-secret')
            .update(hash)
            .digest('hex');
    ...
    async verifyCertificate(certificate) {
        try {
            ...
            const expectedSignature = crypto.createHmac('sha256', certData.issuer + 'cert-secret')
    ...
    async verifyCryptographicProof(result) {
        try {
            ...
            const expectedSignature = crypto.createHmac('sha256', data.nodeId + 'proof-secret')
                .update(recomputedHash)
                .digest('hex');
    ...
    updatePerformanceMetrics(result) {
        ...
        if (result.complexity === 'O(√n)' && result.iterations <= Math.sqrt(result.matrixAnalysis.size) * 15) {
    ...
    getPerformanceMetrics() {
        const complexityVerificationRate = this.performanceMetrics.complexityVerifications.length > 0 ?
    ...
    calculateAverageIterationsToSizeRatio() {
        ...
        const ratios = this.performanceMetrics.complexityVerifications.map(v =>
            v.iterations / Math.sqrt(v.matrixSize)
        );
