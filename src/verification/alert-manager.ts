...
export class TruthAlertManager {
  ...
  constructor(config: TruthTelemetryConfig, logger: ILogger, eventBus: IEventBus) {
    this.config = config;
    this.logger = logger;
  ...
  private generateAlertMessage(rule: AlertRule, metric: TruthMetric): string {
    return (
      `${rule.name}: ${metric.metricType} ${rule.operator} ${rule.threshold} ` +
  ...
  private checkEscalationConditions(alert: TruthAlert, escalation: EscalationLevel): boolean {
    ...
    for (const condition of escalation.conditions) {
      ...
      if (condition.includes('duration') && !this.checkDurationCondition(alert, condition))
        return false;
    }
    return true;
  }
