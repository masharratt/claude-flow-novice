...
export class DashboardExporter {
  ...
  private async generateInterventionTrend(truthMetrics: TruthMetric[]): Promise<DataPoint[]> {
    ...
    truthMetrics.forEach((metric) => {
      ...
      if (
        metric.context.verificationMethod === 'human' ||
        metric.context.verificationMethod === 'hybrid'
      ) {
        data.interventions++;
      }
    });
  ...
  private async generateAgentPerformanceChart(
    agentScores: AgentTruthScore[],
  ): Promise<AgentPerformanceChart[]> {
    return agentScores
  ...
  private async generateErrorDistribution(
    truthMetrics: TruthMetric[],
  ...
  private async generateSystemLoadChart(
    systemMetrics: SystemTruthMetrics,
  ): Promise<SystemLoadChart[]> {
  ...
  private async generateCriticalIssues(data: any): Promise<CriticalIssue[]> {
    ...
    const problematicAgents = agentScores.filter(
      (score) => score.riskAssessment.level === 'critical' || score.riskAssessment.level === 'high',
    );

    if (problematicAgents.length > 0) {
      issues.push({
        id: 'agents-at-risk',
        severity: problematicAgents.some((a) => a.riskAssessment.level === 'critical')
          ? 'critical'
          : 'high',
  ...
  private async analyzePerformanceInsights(data: any): Promise<SystemInsight | null> {
    ...
    const avgScore =
      agentScores.length > 0
        ? agentScores.reduce((sum, s) => sum + s.overallScore, 0) / agentScores.length
        : 0;
  ...
  private calculateOverallHealth(
    ...
    const agentHealth =
      agentScores.length > 0
        ? agentScores.reduce((sum, score) => sum + score.overallScore, 0) / agentScores.length
        : 0.8;
      ...
      panels: [
        {
          ...
          query:
            'SELECT timestamp, AVG(value) FROM truth_metrics WHERE metric_type = "accuracy" GROUP BY hour',
