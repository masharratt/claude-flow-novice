...
class TruthMonitoringServer extends EventEmitter {
  ...
  broadcastTruthEvent(event) {
    ...
    const matchingSubscriptions = Array.from(this.subscriptions.values()).filter((sub) =>
      this.eventMatchesFilter(truthEvent, sub.filter),
    );
  ...
  validateSubscriptionFilter(filter = {}) {
    return {
      event_types: Array.isArray(filter.event_types)
        ? filter.event_types
        : ['truth_change', 'verification_update', 'system_alert'],
      severity_levels: Array.isArray(filter.severity_levels)
        ? filter.severity_levels
