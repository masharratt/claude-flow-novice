...
function calculateAverageConfidence() {
  const verifications = Array.from(verificationStore.verifications.values()).filter(
    (v) => v.status !== 'pending',
  );
...
function calculateTrends(events) {
  ...
  return {
    ...
    peak_hour: Object.keys(hourlyBuckets).reduce(
      (a, b) => (hourlyBuckets[a] > hourlyBuckets[b] ? a : b),
      '0',
    ),
  };
}
