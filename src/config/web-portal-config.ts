...
const defaultConfig: WebPortalConfig = {
  ...
  frontend: {
    ...
    buildPath: process.env.FRONTEND_BUILD_PATH || path.join(process.cwd(), 'src/web/frontend/build'),
    staticPath: process.env.FRONTEND_STATIC_PATH || path.join(process.cwd(), 'src/web/frontend/build'),
  ...
  logging: {
    ...
    outputs: process.env.LOG_OUTPUTS
      ? process.env.LOG_OUTPUTS.split(',') as any
      : ['console'],
    files: {
...
export function validateConfig(config: WebPortalConfig): void {
  ...
  if (config.server.environment === 'production') {
    if (config.security.jwt.secret === generateSecretKey() || config.security.jwt.secret.length < 32) {
  ...
  if (['postgresql', 'mysql'].includes(config.database.type) &&
