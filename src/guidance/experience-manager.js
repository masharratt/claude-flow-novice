...
class ExperienceManager extends EventEmitter {
  ...
  calculateExperienceLevel(userProfile) {
    ...
    const successRate =
      stats.totalCommands > 0 ? stats.successfulCommands / stats.totalCommands : 0;
  ...
  meetsExpertCriteria(stats, activeDays, successRate, complexTasks) {
    ...
    return (
      stats.totalCommands >= expertThresholds.minTotalCommands &&
  ...
  meetsNoviceCriteria(stats, activeDays, complexTasks) {
    ...
    return (
      stats.totalCommands <= noviceThresholds.maxTotalCommands ||
  ...
  countComplexTasks(userProfile) {
    ...
    return userProfile.taskHistory.filter(
      (task) =>
  ...
  calculateSkillProficiency(userProfile) {
    ...
    for (const [categoryName, category] of Object.entries(this.skillCategories)) {
      ...
      for (const skill of category.skills) {
        const skillTasks = userProfile.taskHistory.filter(
          (task) => task.command.includes(skill.replace('-', ' ')) || task.command.includes(skill),
        );
  ...
  async trackMilestone(userProfile, milestoneKey, context = {}) {
    ...
    if (milestone.unlocks) {
      userProfile.learningProgress.unlockedFeatures =
        userProfile.learningProgress.unlockedFeatures || [];
  ...
  generateLearningPath(userProfile) {
    ...
    learningPath.recommendedTutorials = tutorials.filter(
  ...
  calculateProgressToNextLevel(userProfile) {
    ...
    const progress = {
      commands: Math.min(
        stats.totalCommands /
          (nextLevelThresholds.minTotalCommands || nextLevelThresholds.maxTotalCommands),
        1.0,
      ),
      success: Math.min(
        stats.successfulCommands /
          (nextLevelThresholds.minSuccessfulCommands || nextLevelThresholds.maxSuccessfulCommands),
        1.0,
      ),
      days: Math.min(
        this.calculateActiveDays(userProfile) /
          (nextLevelThresholds.minActiveDays || nextLevelThresholds.maxActiveDays),
        1.0,
      ),
    };
    ...
    const tutorials = {
      ...
      expert: [
        { id: 'custom-agents', title: 'Custom Agent Development', estimatedTime: '45 min' },
        {
          ...
          title: 'System Optimization Mastery',
