...
class ContextHelper {
    ...
    identifyTaskType(description, files = [], errors = []) {
        ...
        if (errors.length > 0 || text.includes('bug') || text.includes('fix') || text.includes('error')) {
            return 'debugging';
        }

        if (text.includes('api') || text.includes('endpoint') || text.includes('rest') || text.includes('microservice')) {
            return 'api-development';
        }

        if (text.includes('test') || text.includes('spec') || text.includes('tdd') || text.includes('coverage')) {
            return 'testing';
        }

        if (text.includes('deploy') || text.includes('ci') || text.includes('cd') || text.includes('pipeline')) {
            return 'deployment';
        }

        if (text.includes('performance') || text.includes('optimize') || text.includes('slow') || text.includes('bottleneck')) {
            return 'performance-optimization';
        }

        if (text.includes('web') || text.includes('frontend') || text.includes('react') || text.includes('vue') || text.includes('angular')) {
    ...
    assessComplexity(description, files = []) {
        ...
        const complexKeywords = ['architecture', 'distributed', 'microservice', 'optimization', 'security', 'performance'];
    ...
    assessUrgency(timeConstraints, errors = []) {
        ...
        if (timeConstraints && (timeConstraints.includes('today') || timeConstraints.includes('deadline'))) return 'medium';
        return 'low';
    }
    ...
    recommendAgents(taskType, complexity, urgency) {
        const baseRecommendations = this.knowledgeBase.get(taskType)?.recommendedAgents || ['coder', 'reviewer'];
    ...
    suggestWorkflow(taskType, complexity, urgency) {
        const workflows = {
            ...
            'web-development': {
                high: ['requirements', 'architecture', 'frontend', 'backend', 'integrate', 'test', 'deploy'],
