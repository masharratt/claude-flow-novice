...
export class SERPPatternAnalyst {
  ...
  private validateApiKeyConfig(): void {
    ...
    if (!hasGoogleConfig && !hasDataForSeoConfig) {
      throw new SERPAnalysisError(
        ...
        'No API keys configured. Set GOOGLE_API_KEY + GOOGLE_SEARCH_ENGINE_ID or DATA_FOR_SEO_API_KEY'
      );
    }
  ...
  private sanitizeErrorMessage(message: string): string {
    ...
    sanitized = sanitized.replace(
      /(?:api[_-]?key|api_?secret|access[_-]?token|bearer)[\s]*[=:]\s*[^\s&"']+/gi,
      '[REDACTED_API_KEY]'
    );
  ...
  private async fetchSearchResults(): Promise<SearchResult[]> {
    ...
    throw new SERPAnalysisError(
      SERPAnalysisErrorCode.API_REQUEST_FAILED,
      `All API providers failed: ${errors.map(e => e.message).join('; ')}`,
      { errors }
    );
  }
   ...
   * @private
   */
  private async fetchFromDataForSEO(apiKey: string): Promise<SearchResult[]> {
    try {
      ...
      const response = await axios.post<DataForSEOResponse>(url, payload, {
        headers: {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.code === 'ECONNABORTED') {
          throw new SERPAnalysisError(SERPAnalysisErrorCode.TIMEOUT, 'DataForSEO request timeout');
        }
        ...
        if (axiosError.response?.status === 401) {
          throw new SERPAnalysisError(
            SERPAnalysisErrorCode.API_REQUEST_FAILED,
            'DataForSEO authentication failed - check API key'
          );
        }
      }
   ...
   * @private
   */
  private parseDataForSEOResults(items: DataForSEOItem[]): SearchResult[] {
   ...
   * @private
   */
  private async analyzeDomainAuthority(results: SearchResult[]): Promise<DomainAuthorityPattern> {
    const domains = [...new Set(results.map(r => r.domain))];

    // Fetch backlink counts from SpyFu in parallel
    const backlinkCounts = await Promise.all(
      domains.map(domain => this.fetchBacklinksFromSpyFu(domain))
    );
    ...
    const dominantDomains = domains.filter((d) =>
      results.filter(r => r.domain === d).length > 1
    );
   ...
   * @private
   */
  private async fetchBacklinksFromSpyFu(domain: string): Promise<number> {
    ...
    try {
      ...
      const response = await axios.get<SpyFuDomainOverview>(url, {
        params: {
          domain,
          api_key: spyfuApiKey,
        },
   ...
   * @private
   */
  private async enrichWithScrapedContent(results: SearchResult[]): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < results.length && i < 5; i++) {
          const scraped = scrapedResults.results?.find((r: ScrapedContentResult) => r.url === results[i].url);
        ...
        if (this.config.verbose) {
          console.log(
            `[SERP Analyst] Successfully scraped ${scrapedResults.successCount}/${scrapedResults.totalUrls} URLs`
          );
        }
      } catch (error) {
        ...
        if (this.config.verbose) {
          console.warn(
            `[SERP Analyst] Content scraping skipped: ${errorMsg}`
          );
        }
      }
    } catch (error) {
   ...
   * @private
   */
  private determineApiProvider(): 'google' | 'dataforseo' | 'scraping' {
    const googleApiKey = this.config.googleApiKey || process.env.GOOGLE_API_KEY;
    const dataForSeoApiKey = this.config.dataForSeoApiKey || process.env.DATA_FOR_SEO_API_KEY;
