...
## CFN Loop Enforcement (NEW)

**Auto-Correction System:**
- Validates LOOP/PROCEED/ESCALATE decisions before execution
- Auto-corrects critical violations (LOOP permission requests → execute immediately)
...
// 2. Inject rules at transition
const rules = await injectCFNRulesAtTransition({
  point: 'LOOP_3_RELAUNCH',
  mode: 'standard',
  iteration,
  maxIterations: 10,
...
**Validation Rules:**
1. LOOP without permission - Execute immediately, no approval needed
2. Iteration limits - Enforce max (MVP: 5, Standard: 10, Enterprise: 15)
3. Consensus alignment - LOOP if below threshold, PROCEED if above
...
# View violations
redis-cli GET "cfn:violations:coordinator-id"
```
...
## Loop 3: Hybrid Routing with Specialized Agents

### Three Selection Modes

**1. Automatic (keyword-based matching)**
```bash
node src/cli/hybrid-routing/spawn-workers.js "Build authentication" --max-agents=3

# System automatically selects best agents based on keywords:
# → coder (keywords: implement, build, authentication)
# → security-specialist (keywords: authentication, security)
# → tester (keywords: test, validation)
```
...
```bash
node src/cli/hybrid-routing/spawn-workers.js "OAuth2 implementation" \
  --max-agents=2 \
...
# → coder: Implement OAuth2 with PKCE
# → security-specialist: Audit token handling security
```
...
- analysis (3 agents): code-analyzer, code-quality-validator, perf-analyzer
- core-agents (11 agents): coder, architect, tester, analyst, reviewer, ...
- security (1 agent): security-specialist
- testing (4 agents): interaction-tester, playwright-tester, production-validator, tdd-london-swarm
- devops (1 agent): devops-engineer
- consensus (8 agents): byzantine-coordinator, consensus-builder, raft-manager, ...
- swarm (5 agents): adaptive-coordinator, mesh-coordinator, hierarchical-coordinator, ...
...
```bash
# Agent spawned event
portal:agent:spawned → {agentId, agentType, task}

# Progress updates
portal:agent:update → {agentId, progress, tool}

# Completion event
portal:agent:completed → {agentId, confidence, duration, tokens, cost}

# Swarm completion
portal:swarm:completed → {avgConfidence, totalCost, workers}
```

Monitor at: `http://localhost:3000/agents` (if web portal running)
...
Task Description
    ↓
Check --agents flag?
    ├─ Yes → Use coordinator-specified agents (load from .claude/agents/)
    │        Use --subtasks if provided, else auto-generate
    │
    └─ No → Automatic keyword matching
             ├─ Load 50+ agent definitions
             ├─ Score each agent by keyword overlap
             ├─ Select top N agents
             └─ Auto-generate specialized subtasks

Selected Agents
    ↓
Spawn Workers (z.ai provider)
    ↓
Redis Pub/Sub Coordination
    ↓
Aggregate Confidence Scores
    ↓
Gate Check (≥0.75) → Proceed to Loop 2
```

### Performance

- **Agent discovery**: <100ms (cached after first load)
- **Worker spawning**: ~2s per agent (sequential)
- **Total phase**: 5-10 minutes for 3-5 agents
- **Cost**: $0.50-2.00 per phase (z.ai provider)

### Error Handling

**502 errors**: Automatic retry with exponential backoff (1s, 2s, 4s)
```bash
# Retry tracking in Redis
redis-cli GET "hybrid:worker:1:retries"
# → 2 (max 3 attempts)
```

**Timeout**: 30-minute limit for complex multi-step tasks
```bash
node src/cli/hybrid-routing/spawn-workers.js "Task" --timeout=1800000
```

**Graceful degradation**: Works without Redis/SQLite/Portal (logs warnings)

### Best Practices

1. **Use automatic mode** for standard features (keywords capture intent)
2. **Use coordinator override** to enforce workflow phases (architect → coder → reviewer)
3. **Use full override** for complex/nuanced tasks requiring precise instructions
4. **List agents first** to discover available specialists: `--list-agents`
5. **Monitor via web portal** for real-time progress and debugging
6. **Set whitelist** to restrict available agents in production environments

### Example Workflow

```bash
# Phase: Authentication System (Loop 3)

# Step 1: List available agents
node src/cli/hybrid-routing/spawn-workers.js --agents-by-category | grep security

# Step 2: Spawn specialized agents (coordinator override)
node src/cli/hybrid-routing/spawn-workers.js "Build JWT authentication" \
  --max-agents=4 \
  --agents=architect,coder,security-specialist,tester

# Step 3: Monitor progress via Redis
redis-cli SUBSCRIBE "hybrid:worker:*:complete"

# Step 4: Check results
redis-cli HGETALL "hybrid:task:auth:results"

# Step 5: Calculate gate score
redis-cli EVAL "..." 1 "hybrid:task:auth:results"
# → 0.83 ✅ (passes 0.75 threshold)

# Proceed to Loop 2 (validation)
```
