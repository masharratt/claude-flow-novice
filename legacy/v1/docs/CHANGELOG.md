...
### 🐛 Fixed: Security Scanner Memory Leak
- **Root Cause**: Unclosed file handles, regex state leaks, lack of timeouts in `scripts/security-scan.js`
- **Impact**: Memory accumulation during comprehensive security scans (ourstories-v2 affected)
- **Changes**:
  - Added 5-minute timeout wrapper around all scans
  - Implemented automatic resource cleanup with file handle tracking
  - Replaced sync `fs.readFileSync()` with streaming `fs.createReadStream()`
  - Added 5MB file size limit to prevent OOM
  - Reset regex `lastIndex` state between scans
  - Added MAX_MATCHES=1000 limit for ReDoS prevention
  - Garbage collection hints every 100 files
  - Memory delta logging (warns if >100MB growth)

### 📚 Documentation
- **docs/fixes/SECURITY-SCAN-MEMORY-LEAK-FIX.md**: Complete memory leak analysis and fix details
  - Performance comparison table
  - Testing instructions
  - Distribution checklist

### 🔧 Files Modified
- `scripts/security-scan.js` - Memory leak prevention (timeout, streaming, cleanup)

### 📦 Distribution
- Fixes distributed to all CFN projects via npm publish
