...
export class AgentTruthScorer {
  ...
  private async calculateReliabilityScore(metrics: TruthMetric[]): Promise<number> {
    ...
    const successRate =
      validationScores.filter((score) => score >= 0.8).length / validationScores.length;
  ...
  private async calculateEfficiencyScore(agentData: AgentPerformanceData): Promise<number> {
    const recentTasks = agentData.taskHistory.filter(
      (task) =>
        task.timestamp > new Date(Date.now() - this.config.windowSizes.short * 60 * 60 * 1000),
    );
    ...
    const interventionRate =
      recentTasks.filter((t) => t.interventionRequired).length / recentTasks.length;
  ...
  private async calculateAdaptabilityScore(agentData: AgentPerformanceData): Promise<number> {
    const recentTasks = agentData.taskHistory.filter(
      (task) =>
        task.timestamp >
        new Date(Date.now() - this.config.windowSizes.medium * 24 * 60 * 60 * 1000),
    );
  ...
  private async generatePerformanceWindows(
    ...
    const mediumStart = new Date(
      now.getTime() - this.config.windowSizes.medium * 24 * 60 * 60 * 1000,
    );
    ...
    const longStart = new Date(
      now.getTime() - this.config.windowSizes.long * 7 * 24 * 60 * 60 * 1000,
    );
  ...
  private async assessRisk(
    ...
    const maxSeverity =
      riskFactors.length > 0 ? Math.max(...riskFactors.map((f) => f.severity)) : 0;
  ...
  private getTrendDirection(
    ...
    const recentMetrics = agentData.metrics.filter((m) => m.metricType === metricType).slice(-10); // Last 10 metrics
  ...
  private async updateErrorHistory(
    agentData: AgentPerformanceData,
    metric: TruthMetric,
  ): Promise<void> {
  ...
  private identifyErrorPattern(error: any, errorHistory: ErrorAnalysis[]): string {
    ...
    const recentSimilar = errorHistory.filter(
      (e) => e.errorType === error.type && e.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000), // Last 24 hours
    );
  ...
  private initializeBenchmarks(): void {
    ...
    this.benchmarkScores.set('efficiency', 0.8);
  ...
  getAgentsByRiskLevel(riskLevel: 'low' | 'medium' | 'high' | 'critical'): AgentTruthScore[] {
    return Array.from(this.agentScores.values()).filter(
      (score) => score.riskAssessment.level === riskLevel,
    );
  }
  ...
  getPerformanceStatistics(): {
    ...
    return {
      ...
      atRiskAgents: scores.filter(
        (s) => s.riskAssessment.level === 'high' || s.riskAssessment.level === 'critical',
      ).length,
