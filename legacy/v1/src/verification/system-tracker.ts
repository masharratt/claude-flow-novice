...
export class SystemTruthTracker {
  ...
  private async updateRealTimeMetrics(): Promise<void> {
    ...
    this.systemMetrics.verifiedTasks = this.historicalMetrics.filter(
      (m) => m.validation.isValid,
    ).length;

    // Update critical failures
    const criticalErrors = recentMetrics.filter((m) =>
      m.validation.errors.some((e) => e.severity === 'critical'),
    ).length;
  ...
  private async updateSystemMetricsFromBatch(metrics: TruthMetric[]): Promise<void> {
    ...
    this.systemMetrics.successRate =
      metrics.length > 0 ? (successCount / metrics.length) * 100 : 100;
  }
  ...
  private async updateDistributionMetrics(): Promise<void> {
    ...
    this.systemMetrics.distributionMetrics.accuracyDistribution =
      Object.fromEntries(accuracyRanges);
    ...
    this.systemMetrics.distributionMetrics.complexityDistribution =
      Object.fromEntries(complexityTypes);
  ...
  private async identifyPredictionFactors(
    ...
    if (trend) {
      factors.push({
        ...
        confidence:
          trend.significance === 'high' ? 0.8 : trend.significance === 'medium' ? 0.6 : 0.4,
  ...
  private calculateSystemEfficiency(metrics: TruthMetric[]): number {
    ...
    const automationRate =
      metrics.filter((m) => m.context.verificationMethod === 'automated').length / metrics.length;
  ...
  private calculateValidationHealth(): number {
    ...
    const latencyScore =
      this.systemMetrics.latency > 0
        ? Math.max(0.1, Math.min(1, 5000 / this.systemMetrics.latency))
        : 1;
  ...
  private calculateScoringHealth(): number {
    ...
    const coverageScore =
      this.systemMetrics.agentCount > 0
        ? Math.min(1, this.systemMetrics.activeAgents / this.systemMetrics.agentCount)
        : 1;
    ...
    this.systemTrends.forEach((trend) => {
      if (trend.direction === 'declining' && trend.significance === 'high') {
        recommendations.push(
          `${trend.metric} is declining significantly - immediate attention required`,
        );
      }
    });
