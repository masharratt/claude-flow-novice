...
class GuidanceCommands {
    ...
    try {
      ...
      if (options.detailed) {
        ...
        if (status.learningProgress) {
          console.log(chalk.green('\n🏆 Achievements:'));
          console.log(
            `  Completed Tutorials: ${status.learningProgress.completedTutorials?.length || 0}`,
          );
          console.log(
            `  Mastered Concepts: ${status.learningProgress.masteredConcepts?.length || 0}`,
          );
  ...
  async analyzeContext(options) {
    ...
    try {
      ...
      console.log(
        chalk.green('Analysis complete! Context-specific guidance would be displayed here.'),
      );
