        ...
        return new Promise((resolve, reject) => {
            ...
            if (timeout > 0) {
                timeoutId = setTimeout(() => {
                    if (!killed) {
                        killed = true;
                        console.warn(`⚠️  Process timeout: ${command} ${args.join(' ')} (${timeout}ms)`);
            ...
            proc.stdout.on('data', (data) => {
                if (!killed) {
                    ...
                    if (stdout.length + chunk.length > maxBufferSize) {
                        if (killOnOversize) {
                            killed = true;
                            console.warn(`⚠️  Process output too large: ${command} ${args.join(' ')} (${stdout.length + chunk.length} > ${maxBufferSize} bytes)`);
            ...
            proc.stderr.on('data', (data) => {
                if (!killed) {
                    ...
                    if (stderr.length + chunk.length > maxBufferSize) {
                        if (killOnOversize) {
                            killed = true;
                            console.warn(`⚠️  Process error output too large: ${command} ${args.join(' ')} (${stderr.length + chunk.length} > ${maxBufferSize} bytes)`);
...
class ProcessMonitor {
    ...
    checkMemory() {
        ...
        if (memoryMB > this.maxMemoryMB) {
            ...
            console.error('🔥 EMERGENCY SHUTDOWN - Process killed to prevent system crash');
    ...
    checkDuration() {
        ...
        if (elapsed > this.maxDurationMs) {
            ...
            console.error('🔥 EMERGENCY SHUTDOWN - Process killed to prevent hanging');
...
async function main() {
    ...
    if (!filePath) {
        console.log(`
🔴🟢♻️ UNIFIED POST-EDIT PIPELINE - v3.0.0 (Memory-Safe)
...
Features:
  ...
  ✅ Memory leak protection and auto-kill safeguards
    ...
    console.log(`🛡️  Process limits: ${maxMemoryMB}MB memory, ${(maxDurationMs / 1000).toFixed(0)}s duration`);
