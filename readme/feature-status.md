...
**Last Updated:** 2026-08-19 (cfn-megaplan-fast: program-mode token-lean planner + check-size.sh/extract-sections.sh) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-megaplan-fast | ⚠️ Beta | ✅ 67 | `.claude/skills/cfn-megaplan-fast/` | Token-lean planner: program mode (spec/data/arch/ux once, per-part test-plan + write-plan + Bar A over section extracts), hard artifact byte caps (`cfn-megaplan/bars/check-size.sh`), static-only bars 1 round, opus only spec+arch, no nested spawns, optional /goal driver. Same loop-task hand-off. Known limitations: no ops/tiers/probe; first real-run token measurement pending (AC-7 of its plan). |
...
| Feature | Status | Tests | Location | Description |
|---------|--------|-------|----------|-------------|
| cfn-megaplan orchestrator | ⚠️ Beta | ⚠️ wiring | `.claude/skills/cfn-megaplan/` | Tiered DAG entry point; supersedes cfn-spa-plan. Multi-plan support: shared decision register, cross-plan seam ledger, build-order DAG. PLAN_ persistence gate. Bar B CONDITIONAL-PASS for sibling-blocked plans. §1a actor gate; [OPEN] triage; patch-mode loop-backs. (CHANGELOG.md) |
