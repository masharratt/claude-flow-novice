...
**Last Updated:** 2026-08-19 (cfn-megaplan-fast --part-specs failsafe; bar hardening from curve2026; size caps in megaplan/lite) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-megaplan-fast | ⚠️ Beta | ✅ 67 | `.claude/skills/cfn-megaplan-fast/` | Token-lean planner: program mode (spec/data/arch/ux once, per-part test-plan + write-plan + Bar A over section extracts), hard artifact byte caps (`cfn-megaplan/bars/check-size.sh`), static-only bars 1 round, opus only spec+arch, no nested spawns, optional /goal driver, `--part-specs` failsafe (auto when parts >= 4 or a part's SPEC extract is thin: one 12KB sonnet PARTSPEC per part). Same loop-task hand-off. Known limitations: no ops/tiers/probe; first real-run token measurement pending (AC-7 of its plan). |
