...
| verify-run S008 tool preflight | ✅ Prod | ✅ 27/27 | `.claude/skills/cfn-loop-orchestration-v2/cli/verify-run.sh` | An absent binary used to score GREEN: `bash -c` emits no stdout, so a `... \| wc -l; test -eq 0` check read empty output as zero offenders. Now mode=error/exit 127 via three detectors (requires.tools, CFN_KNOWN_TOOLS in command position, post-run not-found scan) resolved with `type -P`. devDependency binaries excluded by design. |
...
| cfn-workbench | ⚠️ Beta | ✅ 446 | `.claude/skills/cfn-workbench/` | Per-run self-contained HTML progress page + project dashboard (one transit line per run, --live, 12-band cap). Garden section: run as a growing plant (SVG + inline WebGL/LYGIA overlay, hides on context loss). Map, roster, events, staleness pill, watch.sh. |
...
| caveman carve-out guard | ✅ Prod | ✅ 15/15 | `tests/test-caveman-carveout.sh`, `.claude/global/CLAUDE.md`, `.claude/global/references/caveman-controls.md` | Terse output is exempted for subagent prompts, on-disk plan artifacts, AskUserQuestion text, commits and readme. Test pins the guide's rule plus the control claims that live in `caveman-controls.md` (off knob, flag path, intensity not being a size lever). Restores the live flag. |
...
| sourced-path gate | ✅ Prod | ✅ 29 pass | `tests/test-sourced-paths-exist.sh` | CI gate. Resolves every `source` target naming a `.claude` path by replaying the file's assignment prefix from `/tmp`, then requires the file to exist. Catches referrers the skill consolidation left behind. Unresolvable targets SKIP, never PASS |
| sourced-library safety gate | ✅ Prod | ✅ 16/16 | `tests/security/test-sourced-library-safety.sh` | CI gate. A `source`-entry library must not freeze names its callers set: `readonly RED=` killed every caller predeclaring colours, and assigning to a frozen name is equally fatal, so both need a guard. Also sources the checkpoint pair, the only route to that collision |
| env-handling gate | ✅ Prod | ✅ 9/9 | `tests/security/test-env-handling.sh` | CI gate. A `.env` value wrapping onto a continuation line executes as shell when sourced. Knowledge-base approval/review now extract keys with `grep`/`cut`. The fixture proves sourcing DOES fire a planted marker, so the check cannot pass vacuously |
| agent-selection bundle gate | ✅ Prod | ✅ 6/6 | `tests/test-agent-selection-bundle.sh` | CI gate, after `npm ci` so its staleness check can rebuild. `dist/cli.cjs` is committed because consuming projects reach the skill via the `~/.claude/skills` symlink with no `node_modules`. Asserts `100755` in the index, JSON shape, a run under `env -i`, and rebuild-vs-source |
...
**Native-plan-mode capture hook (🚧 Dev, 2026-07-20):** `.claude/hooks/cfn-plan-capture.sh` (PostToolUse on `EnterPlanMode|ExitPlanMode`, registered user-level in `~/.claude/settings.local.json`) closes the gap where native plan mode emits prose and `/cfn-loop-task` finds no manifest, forcing the weak 5E.4 exit gate. On EnterPlanMode it injects the write-plan shaping contract (H1 title, one `## Phase` per lane, Produces/Consumes step table) so the plan is born parseable. On ExitPlanMode it persists the approved plan to `planning/PLAN_<slug>.md` (slug from H1) and orders the manifest chain: `check-produce-consume.sh` → cfn-plan-review → `VERIFY_<slug>.md` (Bar A) → `check-verifiable-static.sh` → `.VERIFY_<slug>.sha256` sidecar → `/cfn-loop-task <slug>`, so the exit gate starts at 5E.0. Bars resolved via `$HOME/.claude` so the chain works from any project; existing `PLAN_<slug>.md` is never clobbered (new plan parked at `.raw_PLAN_*` for model reconcile); every failure exits 0 silently, never wedging plan mode. Test harness `tests/test-plan-capture.sh` (23 cases, all green). Known limit: `check-produce-consume.sh` parses only the first Produces/Consumes table, so multi-phase plans validate only phase 1. 2026-08-22: cfn-plan-review Phase 4 gained state coverage — for each screen/data surface the review lists candidate states (empty, loading, error, partial, stale, no-access incl. not-found vs forbidden) and asks the user once which are binding; selected states require plan coverage plus a test (Alpha Readiness regression row), declined states are recorded as declined so later readers can tell considered-and-cut from never-thought-about. User-confirmed cut because blanket state coverage with tests is sometimes overkill. 2026-08-23: ported three megaplan behaviors into the plan-mode path so ordinary features never route to megaplan — (1) open-item register (Phase 5.7): decisions the plan never made are collected and either asked (BLOCKING, user-visible/shared-state) or recorded with default + revisit trigger (DEFERRED); empty register must say `_none_` so "no open items" is distinguishable from "nobody looked"; added to the evidence floor and output template; (2) new-column safety row in Alpha Readiness: backfill for new non-null columns on existing tables, index on new FK/lookup columns, stated constraint choice; (3) intent confirm: guide Plan Mode Protocol gains the ask-before-writing rule for ambiguous user-visible changes, and cfn-plan-review runs a review-time backstop (one question when plan behavior plausibly mismatches the ask).
