...
**Last Updated:** 2026-08-28 (cfn-workbench transit map reworked for readability: trunk+connector geometry with arrowheads, legend row, compact tallest-column height) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-workbench | ⚠️ Beta | ✅ 237 | `.claude/skills/cfn-workbench/` | Self-contained HTML progress page per loop run. watch.sh re-renders on data change; transit map (trunk+connector SVG, arrowheads, legend, tall-column height) from run-plan + lanes-<slug>.json; lane roster, event feed, staleness pill. Wired into /cfn-loop-task all phases. |
