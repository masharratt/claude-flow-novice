...
**Last Updated:** 2026-08-28 (cfn-workbench: new project dashboard, one transit line per loop run on one auto-refreshing tab) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-workbench | ⚠️ Beta | ✅ 407 | `.claude/skills/cfn-workbench/` | Self-contained HTML progress page per loop run + project dashboard (dashboard.sh): every run in the project as its own transit line on one tab, --live refresh, 12-band cap. Per-run map from run-plan + lanes-<slug>.json; watch.sh re-render; roster, event feed, staleness pill. |
