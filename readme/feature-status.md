...
**Last Updated:** 2026-08-19 (fast: cap-overrun handling fixed after first-run spiral; part-specs auto at 3 parts) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-megaplan-fast | ⚠️ Beta | ✅ 67 | `.claude/skills/cfn-megaplan-fast/` | Token-lean planner: program mode (spec/data/arch/ux once, per-part test-plan + write-plan + Bar A over section extracts), hard artifact byte caps (`cfn-megaplan/bars/check-size.sh`), static-only bars 1 round, opus only spec+arch, no nested spawns, optional /goal driver, `--part-specs` failsafe (auto when parts >= 3 or a part's SPEC extract is thin: one 12KB sonnet PARTSPEC per part). Same loop-task hand-off. Known limitations: no ops/tiers/probe; first real-run token measurement pending (AC-7); 2026-08-19 first run found writer self-compression spiral, prompt now caps self-compression at 1 pass. |
