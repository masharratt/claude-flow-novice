...
**Last Updated:** 2026-09-06 (cfn-fleet workbench bridge: watch --emit-events feeds the HTML dashboard) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-fleet (multi-session coordination) | ✅ Beta | ✅ 246/246 | `.claude/skills/cfn-fleet/` | One master session coordinates N worker sessions via file roster in target repo: claims (file ownership), heartbeats, claim-enforced commits (unclaimed dirty refused, exit 66), flock'd migration-number allocation, tmux spawn with spare sessions for compaction restarts, per-WS scratch postgres, watch events (CHANGE/STALE/DEAD/ALL-DONE) for the Monitor tool. Workbench bridge: `watch --emit-events` feeds cfn-workbench's HTML dashboard (run-plan regen + loop/lane event mapping). Default main-in-place; worktree mode opt-in. Not yet proven in a live fleet run. |
