...
**Last Updated:** 2026-08-27 (cfn-workbench transit map section added: SVG lane/wave map with gate, loop-back and trains, lanes-<slug>.json input, 40 new TDD assertions) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-workbench | ⚠️ Beta | ✅ 237 | `.claude/skills/cfn-workbench/` | Self-contained HTML progress page per loop run. watch.sh re-renders on data change; transit map (SVG lanes/waves/gate/trains) from run-plan + lanes-<slug>.json; lane roster, event feed, staleness pill. Wired into /cfn-loop-task all phases. |
