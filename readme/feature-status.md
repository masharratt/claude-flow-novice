...
| Skill | Status | Tests | Location | Description |
|-------|--------|-------|----------|-------------|
| prompt-optimizer | ✅ Prod | ✅ 101/101 | `prompt-optimizer/` | Provider-agnostic hill-climb prompt optimizer engine. State-isolated, mutation-driven hill climbing with holdout validation gate. Shared engine at `.claude/skills/prompt-optimizer/`, project-local plugin contract via `<project>/.claude/prompt-optimizer/`. 8 engine TS modules (paths, budget, mutator, eval, optimize, rubric-core, source-patcher, types) + 6 test suites (101 tests, +57 from engine fixes). L1: seed template backup to `backups/<target-id>-<ISO>.md` before every overwrite, run report gains "Seed vs Final Diff" section. L2: optional `Target.evalTemperature` with NONDETERMINISTIC SCORING warning. L3: `--holdout-repeats=N` (default 2) for nondeterministic targets; win must hold on every repeat else INCONCLUSIVE. L4: `budget.costFor` no longer throws on unknown models; optional `Target.pricing` for plugin-supplied rates. L5: report notes RUBRIC SATURATED at final total 0. Dogfood plugin at `.claude/prompt-optimizer/` (commit-msg target on kimi-k2.6): 15 fixtures (10 train / 5 holdout), 34 mocked-provider tests, optimized + seed templates tracked. `execute.sh` skill entry point. |
