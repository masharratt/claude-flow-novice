...
**Last Updated:** 2026-09-10 (cfn-fleet dashboard tracking subcommand) | **Version:** 2.21.0 | **Status:** Production
...
| cfn-fleet (multi-session coordination) | ✅ Beta | ✅ 450/450 | `.claude/skills/cfn-fleet/` | One master session coordinates N worker sessions via file roster in target repo: claims (file ownership), heartbeats, claim-enforced commits (unclaimed dirty refused, exit 66), flock'd migration-number allocation, tmux spawn with spare sessions for compaction restarts, mixed engines (claude-sub/glm/codex) with per-pane env, banner verification gate, glm data-credential egress block, per-WS scratch postgres, watch events (CHANGE/STALE/DEAD/ALL-DONE, pane-exit DEAD) for the Monitor tool. Workbench bridge: `watch --emit-events` feeds cfn-workbench's HTML dashboard. `fleet dashboard`: roster-sourced HTML tracking on 127.0.0.1 (VS Code Simple Browser; STALE/DEAD badges, timeline). Default main-in-place; worktree mode opt-in. Not yet proven in a live fleet run. |
