...
curve2026 parts are distinct domains (ci-monitoring, identity, schedule, matches-chat, bookings, kiosk). If one program SPEC goes vague, add a `--part-specs` option (short per-part SPEC, shared DATA/ARCH/UX) rather than re-running per-part arch. BUILT 2026-08-19 as `--part-specs=auto|on|off` (fast SKILL.md Step 1b; auto-on when parts >= 4 or any part's SPEC extract < 3072 bytes; one sonnet PARTSPEC per part, cap 12288, shared DATA/ARCH/UX unchanged).
