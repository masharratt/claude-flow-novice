...
│    ⚠️  BROKEN: Unused, agents don't│
│       claim from queue (see Issue #1)│
│                                     │
│ 7. Spawn agents in waves           │
│    (respecting 40GB budget)         │
│                                     │
│ 8. WAIT for completion             │
│    ⚠️  BROKEN: Uses Redis queue    │
│       polling (see Known Issues #1) │
...
### Issue #1: Task Distribution Architecture Mismatch (CRITICAL)

**Status:** Active bug, blocks production use
**Discovered:** 2025-11-12 integration testing
**Severity:** P0 - BLOCKING

**The Mismatch:**
```javascript
// IMPLEMENTED: Coordinator pushes to queue (lines 167-195)
...
// IMPLEMENTED: Agents spawned with embedded task (lines 272, 287)
Env: [`TASK_PROMPT=${promptText}`, ...]
Cmd: ['node', '/app/dist/cli/index.js', 'agent', 'typescript-specialist', promptText]

// PROBLEM: These patterns are mutually exclusive
// - Coordinator expects agents to claim from queue (RPOP task:queue)
// - Agents receive task via environment and execute immediately
// - Agents never interact with Redis queue
// - Result: Queue never consumed, completion never detected
```

**Impact:**
- Agents complete work successfully (484K tokens processed, fixes applied)
- Coordinator never detects completion (infinite wait loop)
- Progress stuck at "0/16 tasks, 16 queued" indefinitely
- Memory leak (agent containers accumulate without cleanup)
- All integration testing blocked

**Evidence:**
- Integration test: 15+ minutes with no progress detected
- Agent logs: Successful completion, 20 iterations, exit code 0
- Coordinator logs: Polling Redis queue forever (queue length never changes)
- Code analysis: No RPOP/BLPOP calls in agent execution flow
- Confidence level: 0.95

**Correct Pattern (Post-Fix):**
```javascript
// Coordinator spawns agents with environment variables (keep current pattern)
docker.createContainer({
  Env: [`TASK_PROMPT=${promptText}`, `AGENT_ID=${agentId}`],
  Cmd: ['node', '/app/dist/cli/index.js', 'agent', 'typescript-specialist', promptText]
});

// Coordinator waits by polling Docker container status (replace queue polling)
...
**Fix Priority:** P0 - Required before any test suite work or production deployment

**Estimated Effort:** 2-3 hours
- Remove Redis queue operations (lines 167-195)
- Replace waitForCompletion() logic (lines 296-350)
- Add container status tracking
- Test with integration suite

**See:** `planning/docker/SESSION_2025-11-12_FINDINGS.md` for detailed root cause analysis
