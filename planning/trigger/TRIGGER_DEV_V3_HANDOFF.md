...
**Date:** 2025-11-22  
**Owner:** Codex  
**Scope:** Migrate CFN Loop to trigger.dev v3 worker (non-shim), ensure North Star Test 2 (5-iteration) passes against a real v3 stack.
...
1) **Real task implementations**  
   - Port v2 jobs/workflows (loop3-agent, loop2-validator, gate-check, product-owner orchestration) to `@trigger.dev/sdk/v3` tasks.  
...
1) **Implement real logic in v3 tasks:**
   - loop3-agent: spawn/execute agent (CLI or equivalent), return real AgentResult.
   - gate-check: compute pass rate from agent results.
   - loop2-validator: spawn validators, aggregate scores.
