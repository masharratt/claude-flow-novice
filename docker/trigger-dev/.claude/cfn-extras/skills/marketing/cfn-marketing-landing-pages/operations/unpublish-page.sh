...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "$PAYLOAD" \
  "$N8N_BASE_URL/webhook/landing-pages-unpublish" 2>/dev/null || echo -e "\n000")
  ...
  200|201)
    ...
    if ! echo "$BODY" | jq empty 2>/dev/null; then
      ...
      echo "{
        \"page_id\": \"$PAGE_ID\",
        \"status\": \"unpublished\",
        \"reason\": $(if [[ -n "$REASON" ]]; then echo "\"$REASON\""; else echo "null"; fi),
        \"unpublished_at\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",
        \"previous_url\": \"https://pages.example.com/page\"
      }"
    else
      echo "$BODY"
    fi
    ...
    ;;
  400)
    echo '{"error": "Bad request - page not published", "code": "API_BAD_REQUEST", "details": '"${BODY:-null}"'}' >&2
    exit 1
    ;;
  401)
