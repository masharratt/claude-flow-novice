...
import { sendEvent, getRunStatus, RunStatus } from '../../trigger-dev-client';
import { CFNLoopPayload, CFNMode, getThresholdConfig } from '../../src/types/cfn-types';
...
describe.skipIf(!WORKER_RUNNING)('North Star Test 3: Real Agent Execution', () => {
  ...
  const MODE: CFNMode = 'mvp'; // Lower thresholds for faster test
  ...
  it('should create deliverable file (if completed)', async () => {
    ...
    try {
      ...
      console.log(`⚠️ Deliverable not found: ${(err as Error).message}`);
    }
  });
});
