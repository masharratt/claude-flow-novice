 ...
 */
function spawnAgentVulnerable(agentType: string, taskId: string): string {
 ...
 */
function spawnAgentSecure(agentType: string, taskId: string): string {
...
describe('Security: Command Injection Prevention (CVSS 7.5)', () => {
  ...
  describe('spawnAgentSecure implementation', () => {
    ...
    it('should reject command injection in secure function', () => {
      const maliciousTaskId = '$(whoami)';
      expect(() => spawnAgentSecure('backend-developer', maliciousTaskId)).toThrow(
