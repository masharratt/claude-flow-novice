...
function createLoop3Payload(overrides = {}) {
    return {
...
describe('CFN Agent Job', () => {
    ...
    describe('Agent ID Generation', () => {
        it('should generate unique agent IDs', () => {
            const generateAgentId = (agentType) => {
    ...
    describe('Confidence Scoring', () => {
        const calculateConfidence = (testResults) => {
    ...
    describe('Deliverables Extraction', () => {
        const extractFilesFromOutput = (output) => {
            const fileMatches = output.match(/(?:modified|created|updated):\s*(.+)/gi);
            if (!fileMatches)
                return [];
            return fileMatches.map((match) => match.replace(/(?:modified|created|updated):\s*/i, '').trim());
        };
        const extractSummaryFromOutput = (output) => {
...
//# sourceMappingURL=cfn-agent.test.js.map