...
class CodeAnalyzer {
   ...
   */
  countLinesInFunction(functionName: string): number {
   ...
   */
  calculateCyclomaticComplexity(functionName: string): number {
    const functionRegex = new RegExp(`\\b${functionName}\\s*[=:].*=>\\s*{`, 'i');
    let startLine = -1;
   ...
   */
  countTryBlocks(functionName: string): { total: number; duplicates: number } {
    const functionRegex = new RegExp(`\\b${functionName}\\s*[=:].*=>\\s*{`, 'i');
    let startLine = -1;
    ...
    if (startLine === -1) return { total: 0, duplicates: 0 };
   ...
   */
  calculateNestingLevel(functionName: string): number {
    const functionRegex = new RegExp(`\\b${functionName}\\s*[=:].*=>\\s*{`, 'i');
    let startLine = -1;
