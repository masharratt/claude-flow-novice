...
test_redis_client_connectivity() {
    ...
    # This ensures the fix works correctly without relying on fallback variables.
    # If REDIS_HOST/PORT were also set, the test would pass even if CFN_REDIS_HOST was broken.
    docker run -d \
...
test_heartbeat_reporting() {
    ...
    # THEN: Heartbeat exists in Redis
    ...
    if [ -n "$HEARTBEAT" ] && [ "$STATUS" = "alive" ]; then
        ...
        log_fail "Heartbeat not found in Redis"
        return 1
    fi
}
