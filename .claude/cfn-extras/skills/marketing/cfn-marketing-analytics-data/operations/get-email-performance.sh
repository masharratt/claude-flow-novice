...
# Get email campaign performance analytics via n8n workflow
...
CAMPAIGN_IDS_ARRAY=$(if [[ -n "$CAMPAIGN_IDS" ]]; then echo "$CAMPAIGN_IDS" | jq -R 'split(",")'; else echo "[]"; fi)
METRICS_ARRAY=$(if [[ -n "$METRICS" ]]; then echo "$METRICS" | jq -R 'split(",")'; else echo "[]"; fi)

PAYLOAD=$(jq -n \
  --arg startDate "$START_DATE" \
  --arg endDate "$END_DATE" \
  --argjson campaignIds "$CAMPAIGN_IDS_ARRAY" \
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "$PAYLOAD" \
  "${N8N_BASE_URL}/webhook/marketing/analytics/email-performance" || echo -e "\n000")
