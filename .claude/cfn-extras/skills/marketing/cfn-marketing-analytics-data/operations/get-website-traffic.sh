...
# Get website traffic analytics via n8n workflow
...
DIMENSIONS_ARRAY=$(if [[ -n "$DIMENSIONS" ]]; then echo "$DIMENSIONS" | jq -R 'split(",")'; else echo "[]"; fi)

PAYLOAD=$(jq -n \
  --arg startDate "$START_DATE" \
  --arg endDate "$END_DATE" \
  --argjson metrics "$METRICS_ARRAY" \
  --argjson dimensions "$DIMENSIONS_ARRAY" \
  '{
    startDate: $startDate,
    endDate: $endDate,
    metrics: $metrics,
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "$PAYLOAD" \
  "${N8N_BASE_URL}/webhook/marketing/analytics/website-traffic" || echo -e "\n000")
