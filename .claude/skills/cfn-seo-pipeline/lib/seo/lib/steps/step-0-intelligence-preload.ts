...
} from '../ruvector/pre-research-query';
import {
  ConditionalStepExecutor,
  type StepExecutionPlan,
  createStandardStepExecutor,
} from '../conditional-step-executor';
...
export interface Step0Config {
  ...
  /** SEO Query Manager for RuVector pre-research (optional) */
  seoQueryManager?: SEOQueryManager;

  /** Enable RuVector pre-research cache lookup */
  enableRuVectorPreResearch?: boolean;
}
...
export interface Step0Result {
  ...
  /** RuVector pre-research result (if enabled) */
  ruVectorPreResearch?: PreResearchResult;

  /** Step execution plan based on cached data */
...
export async function executeStep0(
  ...
  // Step 0.5: RuVector Pre-Research Query (if enabled)
  let ruVectorPreResearch: PreResearchResult | undefined;
  let executionPlan: StepExecutionPlan | undefined;

  if (config.enableRuVectorPreResearch && config.seoQueryManager) {
    if (config.verbose) {
      console.log('[Step 0.5] Querying RuVector for cached research...');
    }

    try {
      const preResearchGatherer = new PreResearchIntelligenceGatherer(config.seoQueryManager);
      const preResearchQuery: PreResearchQuery = {
        keyword: context.task.targetKeyword,
        niche: context.task.industry || 'general',
        competitorDomains: context.task.competitorDomains,
        clusterId: context.task.clusterId,
      };

      ruVectorPreResearch = await preResearchGatherer.gatherIntelligence(preResearchQuery);

      if (config.verbose) {
        console.log(`[Step 0.5] Cache status: ${JSON.stringify(ruVectorPreResearch.cacheStatus)}`);
        console.log(`[Step 0.5] Estimated savings: ${ruVectorPreResearch.estimatedSavings}`);
      }

      // Create execution plan based on cached data
      const stepExecutor = createStandardStepExecutor();
      executionPlan = stepExecutor.createExecutionPlan(ruVectorPreResearch);

      if (config.verbose) {
        console.log('[Step 0.5] Execution plan created:');
        console.log(`  - Step 1 (Keyword Research): ${executionPlan.step1_keyword_research.skip ? 'SKIP' : 'RUN'}`);
        console.log(`  - Step 2.5 (Competitor Analysis): ${executionPlan.step2_5_competitor_analysis.skip ? 'SKIP' : 'RUN'}`);
        console.log(`  - Step 3.5 (SERP Analysis): ${executionPlan.step3_5_serp_analysis.skip ? 'SKIP' : 'RUN'}`);
        console.log(`  - Step 4 (Research): RUN (supplement from cache: ${executionPlan.step4_research.supplementFromCache})`);
      }

      // Populate context with cached data
      if (ruVectorPreResearch.cachedData) {
        context.cachedResearch = {
          keywordResearch: ruVectorPreResearch.cachedData.keywordResearch,
          competitorIntelligence: ruVectorPreResearch.cachedData.competitorIntelligence,
          serpPatterns: ruVectorPreResearch.cachedData.serpPatterns,
          expertSources: ruVectorPreResearch.cachedData.expertSources,
          statistics: ruVectorPreResearch.cachedData.statistics,
          contentPatterns: ruVectorPreResearch.cachedData.contentPatterns,
        };
      }
    } catch (error) {
      if (config.verbose) {
        console.warn('[Step 0.5] RuVector pre-research failed, continuing without cache:', error);
      }
      // Continue without cached data - not a fatal error
    }
  }
  ...
  return {
    ...
    ruVectorPreResearch,
    executionPlan,
    estimatedSavings: ruVectorPreResearch?.estimatedSavings,
  };
}
