...
LOG_PATH="${PROJECT_ROOT}/.artifacts/logs/ruvector-failure-analysis.log"
...
echo "[RuVector] Analyzing agent failures from last $DAYS days" | tee -a "$LOG_PATH"
...
if [ -z "$FAILED_AGENTS" ]; then
    echo "[RuVector] No failures found in last $DAYS days" | tee -a "$LOG_PATH"
    exit 0
fi

FAILURE_COUNT=$(echo "$FAILED_AGENTS" | wc -l)
echo "[RuVector] Found $FAILURE_COUNT failed agent executions" | tee -a "$LOG_PATH"
...
SQL_EOF
                ...
                echo "[RuVector] Updated pattern: $EXISTING_ID (count: $NEW_COUNT)" | tee -a "$LOG_PATH"
...
SQL_EOF
                ...
                echo "[RuVector] Created pattern: $PATTERN_ID ($AGENT_TYPE / $FAILURE_MODE)" | tee -a "$LOG_PATH"
            fi
        fi
    fi
done

echo "[RuVector] Created $PATTERNS_CREATED new failure patterns" | tee -a "$LOG_PATH"
...
if [ -n "$HIGH_IMPACT" ]; then
    echo "" | tee -a "$LOG_PATH"
    echo "[RuVector] High-impact failure patterns (>= $MIN_OCCURRENCES occurrences):" | tee -a "$LOG_PATH"
...
echo "" | tee -a "$LOG_PATH"
echo "[RuVector] Searching for recovery strategies..." | tee -a "$LOG_PATH"
...
echo "[RuVector] Identified $RECOVERIES_FOUND recovery strategies" | tee -a "$LOG_PATH"
...
echo "" | tee -a "$LOG_PATH"
echo "[RuVector] Failure Analysis Summary:" | tee -a "$LOG_PATH"
