...
```
/cfn-megaplan-fast "<program or feature task>" [--parts=auto|<n>|"B0,B1,B2"] [--unattended]
```
...
planning/<prog>__<part>/              TEST_ PLAN_ VERIFY_ .VERIFY_*.sha256                  (one dir per part; slug = <prog>__<part>)
```
...
```
Node deps: spec → data → (arch ∥ ux) → plan_review → per part: test_plan → write_plan → bar_b. `data`/`ux` skipped when their flag is `no`: substitute the literal `Input DATA: ABSENT (db=no)` in downstream prompts.

## Caps (bytes; canonical in `profiles/fast.json` `.caps`, read by `check-size.sh`)

| SPEC | DATA | ARCH | UX | REVIEW | TEST | PLAN | VERIFY | MEGAPLANFAST |
|---|---|---|---|---|---|---|---|---|
| 24576 | 32768 | 32768 | 32768 | 16384 | 24576 | 40960 | 40960 | 16384 |
...
- **P1 test_plan:** `tester`, sonnet, `cfn-test-plan` light. Inputs = `lib/extract-sections.sh planning/<prog>/SPEC_<prog>.md <part>` (and DATA/ARCH/UX likewise), written to `planning/<prog>__<part>/.in/` so the agent reads files, not pasted blobs. Output TEST_<slug>.md. Size-check.
...
- Re-running spec/arch per part. That is the 60% the baseline wasted.
