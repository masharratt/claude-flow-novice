...
## Relationship to db-query

`db-query` is the canonical entry point for running project DB queries per CLAUDE.md (`./.claude/skills/db-query/execute.sh --sql "..."`). It is generated by `supabase-schema-sync` from the live schema, so it carries current table/column context. `cfn-parameterized-queries` is the parameterized, sanitized SQL execution layer underneath: it validates identifiers and binds values to prevent injection. Use db-query as the documented path; route any hand-built SQL through this layer for injection safety. Raw `psql` is discouraged by CLAUDE.md. After a migration, re-run `supabase-schema-sync` to regenerate db-query.
