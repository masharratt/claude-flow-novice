...
//!
//! ## Excluded Directories (see EXCLUDED_DIRS constant - 56 patterns):
...
//! - Project-specific: .artifacts, worktrees, .worktrees, .codesearch, .archive, archive, .archived, archived
...
use codesearch::store_qdrant::QdrantStore;
use codesearch::store_memgraph::MemgraphStore;
...
const EXCLUDED_DIRS: &[&str] = &[
    ...
    "worktrees",         // CFN agent git worktrees (ephemeral repo clones, dup source + nested node_modules)
    ".worktrees",        // Hidden worktrees variant
    ".codesearch",         // CodeSearch local index (avoid self-indexing)
    ".fastembed_cache",  // FastEmbed ONNX model cache (binary weights)
    ".ruvector",         // Ruvector SQLite binary files
...
impl IndexCommand {
    pub fn new(
        ...
        memgraph_url: Option<&str>,
        memgraph_user: Option<&str>,
        memgraph_password: Option<&str>,
        use_openai: bool,
    ) -> Result<Self> {
        let index_path = get_codesearch_dir()?;
        let embeddings_manager = EmbeddingsManager::new_with_backend(&index_path, use_openai)?;
        let search_engine = SearchEngine::new(&index_path)?;
        let store = SqliteStore::new(&index_path.join("index.db"))?;
        let store_v2 = StoreV2::new(&get_database_path()?)?;

        // Create tokio runtime once, shared by all async stores
        ...
        let pgvector_store = if let Some(url) = pg_url {
            ...
            match rt.block_on(PgvectorStore::new(url)) {
                ...
                Err(e) => {
                    warn!("Failed to connect to pgvector: {}. Skipping.", e);
                    None
                }
            }
        } else {
        ...
        // Initialize Qdrant store if URL provided
        let qdrant_store = if let Some(url) = qdrant_url {
            info!("Initializing Qdrant store with URL: {}", url);
            match rt.block_on(QdrantStore::new(url)) {
                ...
                Err(e) => {
                    warn!("Failed to connect to Qdrant: {}. Skipping.", e);
                    None
                }
            }
        } else {
        ...
        // Initialize Memgraph store if URL provided
        let memgraph_store = if let Some(url) = memgraph_url {
            let user = memgraph_user.unwrap_or("");
            let pass = memgraph_password.unwrap_or("");
            info!("Initializing Memgraph store with URL: {}", url);
            match rt.block_on(MemgraphStore::new(url, user, pass)) {
                Ok(store) => {
                    ...
                    let _ = rt.block_on(store.upsert_project(
                        &project_name,
                ...
                Err(e) => {
                    warn!("Failed to connect to Memgraph: {}. Skipping.", e);
                    None
                }
            }
        } else {
        ...
        let tokio_runtime = if pgvector_store.is_some() || qdrant_store.is_some() || memgraph_store.is_some() {
            Some(rt)
        } else {
            None
        };
            ...
            .filter(|e| {
                ...
                if !self.file_types.is_empty() {
                    if let Some(ext) = e.path().extension() {
                        ...
                        if !self.file_types.iter().any(|t| t.eq_ignore_ascii_case(&ext_str)) {
                            return false;
                        }
                    } else {
                        // No extension — skip unless user explicitly asked for extensionless files
                        return false;
                    }
                }
    ...
    fn process_files(&mut self, files: Vec<PathBuf>) -> Result<IndexStats> {
        ...
        // Recreate ONNX model every BATCH_SIZE files to prevent arena memory leak
        const BATCH_SIZE: usize = 50;
        for (i, file_path) in files.iter().enumerate() {
            // Reset ONNX model every BATCH_SIZE files to release arena memory
            if i > 0 && i % BATCH_SIZE == 0 {
                info!("Resetting ONNX model after {} files to release memory", i);
                self.embeddings_manager = EmbeddingsManager::new_with_backend(
                    &self.index_path, self.use_openai
                )?;
            }
    ...
    fn process_file(
        ...
        if self.is_file_in_index(file_path)? {
            ...
            // Clean up Qdrant entries for this file
            if let (Some(ref qdrant_store), Some(ref rt)) = (&self.qdrant_store, &self.tokio_runtime) {
                if let Err(e) = rt.block_on(qdrant_store.delete_file_entities(&file_path_str, &project_root_str)) {
                    warn!("Failed to delete file entities from Qdrant: {}", e);
                }
            }

            // Clean up Memgraph entries for this file
            if let (Some(ref memgraph_store), Some(ref rt)) = (&self.memgraph_store, &self.tokio_runtime) {
                if let Err(e) = rt.block_on(memgraph_store.delete_file_graph(&file_path_str, &project_root_str)) {
                    warn!("Failed to delete file graph from Memgraph: {}", e);
                }
            }
        }
        ...
        if let (Some(ref embeddings), Some(ref qdrant_store), Some(ref rt)) = (&embeddings, &self.qdrant_store, &self.tokio_runtime) {
            ...
            let store_entities: Vec<codesearch::store_v2::Entity> = extraction_result.entities.iter()
                .enumerate()
                .map(|(i, entity)| codesearch::store_v2::Entity {
                    id: entity_ids[i],
                    kind: match entity.kind {
                        crate::extractors::EntityKind::Function => codesearch::schema_v2::EntityKind::Function,
                        crate::extractors::EntityKind::Method => codesearch::schema_v2::EntityKind::Method,
                    ...
                    doc_comment: extract_doc_comment(&entity.signature),
                    attributes: None,
                    metadata: Some(serde_json::to_string(&entity.metadata).unwrap_or_default()),
                    project_root: project_root_str.clone(),
                    created_at: chrono::Utc::now(),
                    updated_at: chrono::Utc::now(),
                })
            ...
            match rt.block_on(qdrant_store.upsert_batch(&store_entities, &embeddings, &project_root_str)) {
                Ok(count) => debug!("Stored {} entities in Qdrant", count),
                Err(e) => warn!("Failed to store entities in Qdrant: {}", e),
            }
        }

        // Store entities + references in Memgraph (if available)
        if let (Some(ref memgraph_store), Some(ref rt)) = (&self.memgraph_store, &self.tokio_runtime) {
            // Upsert file node
            let language = self.detect_language(file_path).unwrap_or_else(|_| "unknown".to_string());
            let file_hash = self.calculate_file_hash(file_path).unwrap_or_default();
            let _ = rt.block_on(memgraph_store.upsert_file(
                &file_path_str, &project_root_str, &language, &file_hash,
            ));

            // Upsert entity nodes
            for (i, entity) in extraction_result.entities.iter().enumerate() {
                let kind_str = entity.kind.as_str();
                let vis_str = format!("{:?}", entity.visibility);
                let _ = rt.block_on(memgraph_store.upsert_entity(
                    entity_ids[i],
                    &entity.name,
                    kind_str,
            ...
            // Upsert reference edges
            let ref_tuples: Vec<(i64, String, String, String)> = extraction_result.references.iter()
                .enumerate()
                .filter_map(|(i, reference)| {
                    if i < entity_ids.len() {
                        Some((
                            ...
                            format!("{:?}", reference.ref_kind).to_lowercase(),
            ...
            if !ref_tuples.is_empty() {
                match rt.block_on(memgraph_store.upsert_references_batch(&ref_tuples)) {
                    Ok(count) => debug!("Created {} reference edges in Memgraph", count),
                    Err(e) => warn!("Failed to create reference edges in Memgraph: {}", e),
                }
            }
        }
    ...
    fn generate_entity_embeddings(&self, entities: &[Entity], source: &str) -> Result<Vec<Vec<f32>>> {
        let source_lines: Vec<&str> = source.lines().collect();
        // BGE-small-en-v1.5 max sequence = 512 tokens (~2000 chars).
        // Truncate to 1500 chars to avoid wasting ONNX arena memory.
        const MAX_EMBED_CHARS: usize = 1500;
        let texts: Vec<String> = entities.iter()
            .map(|e| {
                let declaration = format!("{}:{}:{}", e.kind.as_str(), e.name, e.signature);
                let start = e.line.saturating_sub(1);
                let end = (start + 30).min(source_lines.len());
