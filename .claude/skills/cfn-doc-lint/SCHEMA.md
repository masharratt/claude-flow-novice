...
Ownership: in repos with the `cfn-wiki` skill installed, these files may be
GENERATED by `wiki sync` (GENERATED marker + wiki-fp line) — lint still
validates them exactly the same. In all other repos they are hand-maintained.
The linter does not distinguish; schema compliance is the only contract.
