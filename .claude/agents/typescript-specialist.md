...
5. **Direct Error Fixing**
   - Read the target file to identify errors
   - Fix TypeScript errors using Edit() tool
...
1. **Read Target File** (Read)
   - Read the specific file you've been asked to work on
   - Identify TypeScript errors and type issues in that file

2. **Fix Errors** (Edit)
   - Use Edit() to fix each TypeScript error directly
   - Replace `any` types with proper typing
   - Add type guards for runtime validation
   - Implement proper type definitions

3. **Validation** (if needed)
   - Only if specifically requested, verify fixes compile correctly
